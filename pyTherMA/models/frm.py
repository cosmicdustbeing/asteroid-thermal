# frm.py
"""
This module implements the FRM thermal model as a class.
It accepts a ShapeModel instance and uses its facets’ solar incidence angles
to compute a temperature for each facet based on the FRM prescription.
"""
...
class FRM:
    def __init__(self, shape_model, A, epsilon, r_au, ss_lat=0):
        """
        Initialize the FRM thermal model.
    ...
    def subsolar_temperature(self):
        """
        Compute the subsolar temperature T_ss using the FRM formula:
        T_ss = [ ( (1-A) * S_SOLAR ) / (pi * epsilon * SIGMA * r_m^2) ]^(1/4)          
        Returns:
    ...
    def temperature_distribution(self):
        ...
        T = T_ss * (cos(lat) * cos(lon))^(1/4), if (cos(lat)*cos(lon)) > 0,
        and T = 0 if the product is <= 0 (i.e. facet not illuminated).
        ...
        #cos_lat = np.cos(sn_lat-self.ss_lat_rad)
        cos_lat = np.sin(sn_lat) * np.sin(self.ss_lat_rad) + np.cos(sn_lat) * np.cos(self.ss_lat_rad)
        
        # ensure finite
        effective_cos = np.where(cos_lat > 0, cos_lat, 0)
        temperatures = self.T_ss * (effective_cos**0.25)

        self.shape_model.temperatures = temperatures
        return temperatures

    def frm_flux(self, diam_eff, wavelength, Delta_au, so_lat=0):
        """
        Compute the integrated flux from the FRM model at a given wavelength or array of wavelengths.
        Parameters:
        ...
        # Retrieve the temperatures computed by FRM.
        T = self.T_surf
